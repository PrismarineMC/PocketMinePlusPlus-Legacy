...
class Server
{
    ...
    public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath)
    {
        ...
        try {
            ...
            $this->logger->info("Loading pm-plus-plus.yml...");

            if (!\file_exists($this->dataPath . "pm-plus-plus.yml")) {
                $content = \file_get_contents($this->filePath . "src/pocketmine/resources/pm-plus-plus.yml");
                @\file_put_contents($this->dataPath . "pm-plus-plus.yml", $content);
            }

            $this->configuration = new Config($this->dataPath . "pm-plus-plus.yml", Config::YAML, []);

            if ($this->configuration->get("all-items-in-creative", true) == true) {
                $this->allItemsInCreative();
            } else {
    ...
    public function allItemsInCreative()
    {
        Item::clearCreativeItems();
        foreach ($this->all_items as $id) {
            $id2 = "0";
            $pos = strrpos($id, ":");
            if ($pos === \false) {
                $id1 = $id;
            ...
            if (is_numeric($id1))
                if (is_numeric($id2))
                    Item::addCreativeItem(Item::get($id1, $id2));
        }
    }
